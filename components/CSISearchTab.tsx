...
export default function CSISearchTab({ onSelectCode }: CSISearchTabProps) {
  ...
  const getConfidenceBadgeColor = (confidence: number) => {
    // Grayscale badges as per user preference
    if (confidence >= 0.8) return 'bg-gray-200 text-gray-800 border-gray-300';
    if (confidence >= 0.5) return 'bg-gray-100 text-gray-700 border-gray-300';
        ...
        {breadcrumb.map((item, index) => {
          ...
          return (
            <div key={index} className="flex items-center gap-1">
              <div className="flex items-center gap-1.5 bg-white rounded px-2 py-1 border border-gray-200">
                <span
    ...
    <div className="p-4 space-y-4">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm font-medium text-gray-600">Fuzzy Search</span>
            <div className="relative group">
              ...
              <div className="absolute right-0 top-6 w-64 bg-white border border-gray-200 shadow-lg text-zinc-900 text-xs rounded-lg p-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-10 pointer-events-none">
                <div className="relative">
                  <div className="absolute -top-3 right-2 w-2 h-2 bg-white transform rotate-45 border-l border-t border-gray-200"></div>
                  {fuzzySearch
            ...
            <button
              ...
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-zinc-500 focus:ring-offset-2 focus:ring-offset-white ${
                fuzzySearch ? 'bg-zinc-900' : 'bg-gray-300'
              }`}
        ...
        <div className="space-y-2">
              ...
              <div className="space-y-2 max-h-[calc(100vh-400px)] overflow-y-auto">
                {results.map((result) => (
                  <button
                    ...
                    className="w-full text-left p-3 bg-white border-2 border-gray-200 rounded-lg hover:border-zinc-900 hover:shadow-md transition-all group"
                  >
                    <div className="space-y-2">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="px-2 py-0.5 bg-gray-100 text-gray-700 text-xs font-medium rounded border border-gray-300">
                            Level {result.level}
                          </span>
                        </div>
                        <p className="text-sm font-medium text-gray-700 group-hover:text-zinc-900">
                          {result.title}
                        </p>
                      </div>
            ...
            <div className="space-y-3">
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="px-2 py-0.5 bg-zinc-900 text-white text-xs font-medium rounded">
                    AI
                  </span>
                </div>
                <p className="text-xs text-gray-600">
              ...
              <div className="space-y-2 max-h-[calc(100vh-450px)] overflow-y-auto">
                {aiMatches.map((match, index) => (
                  <button
                    ...
                    className="w-full text-left p-4 bg-white border-2 border-gray-200 rounded-lg hover:border-zinc-900 hover:shadow-md transition-all group"
                  >
                    <div className="flex items-start gap-3">
                      <div className="flex-shrink-0 w-8 h-8 bg-zinc-900 text-white rounded-full flex items-center justify-center font-bold text-sm">
                        {index + 1}
                      </div>
                      <div className="flex-1 min-w-0 space-y-2">
                        <div>
                          ...
                          <p className="text-sm font-medium text-gray-700 group-hover:text-zinc-900 mb-2">
                            {match.title}
                          </p>
                        </div>
          ...
          {results.length === 0 && aiMatches.length === 0 && !isAIMatching && !aiError && (
            <div className="text-center py-8">
              ...
              <h3 className="mt-3 text-sm font-medium text-zinc-900">No results found</h3>
      ...
      {query.length < 2 && results.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="mt-4 text-sm font-medium text-zinc-900">Search CSI Codes</h3>
