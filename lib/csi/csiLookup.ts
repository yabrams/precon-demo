/**
 * CSI MasterFormat Lookup Utilities
 *
 * Provides search, filtering, and navigation functions for CSI MasterFormat 2018 data.
 */
...
/**
 * Get all CSI codes as a flat array
 */
 ...
 * Validate a CSI code
 */
export function validateCode(codeString: string): CSIValidationResult {
  const code = getCodeByCode(codeString);

  if (!code) {
    // Try to find suggestions
    const suggestions = searchCSICodes({
      query: codeString,
      limit: 5,
      caseSensitive: false,
    }).map((result) => result.code.code);

    return {
      isValid: false,
      error: `Code "${codeString}" not found in CSI MasterFormat 2018`,
 ...
 */
export function getDatasetStats(): CSIDatasetStats {
  const allCodes = getAllCSICodes();
...
/**
 * Get codes that match a specific pattern
 * Useful for finding related codes (e.g., all "03 30" codes)
 */
export function getCodesByPattern(pattern: string): CSICode[] {
  const allCodes = getAllCSICodes();
  const regex = new RegExp(pattern, 'i');
  return allCodes.filter((code) => regex.test(code.code) || regex.test(code.title));
}
